...
from .aldnoah_taildata import manifest_filename, resolve_record
...
#   u32 payload_count
#   repeat payload_count:
#     u16 stored_name_len + stored_name
#     u8  idx_marker      -> target container
#     u32 entry_off       -> target IDX slot offset
#     u8  comp_marker
#     u32 payload_size + payload   (the file itself, nothing appended)
...
class AldnoahPackageWriter:
    ...
    def payload_for_entry(self, entry: "PayloadEntry") -> Tuple[bytes, dict]:
        """
        The payload bytes and the slot they belong to, kept separate
        """
        if entry.record is not None:
            with open(entry.source_path, "rb") as handle:
                return handle.read(), entry.record
        return self.payload_with_record(entry.source_path)

    def payload_with_record(self, source_path: str) -> Tuple[bytes, dict]:
        """
        Read a staged file and look up the slot it targets

        The record comes from the taildata manifest the unpacker wrote
        """
        ...
        *,
        ...
        with open(save_path, "wb") as handle:
            ...
            for entry in payload_entries:
                blob, record = self.payload_for_entry(entry)
                self.write_the_string(handle, entry.stored_name, 2)
                handle.write((int(record["idx_marker"]) & 0xFF).to_bytes(1, "little"))
                handle.write((int(record["entry_off"]) & 0xFFFFFFFF).to_bytes(4, "little"))
                handle.write((1 if record.get("comp_marker") else 0).to_bytes(1, "little"))
                handle.write(len(blob).to_bytes(4, "little"))
                handle.write(blob)
...
class ModCreatorWindow(tk.Toplevel):
    ...
    def build_gui(self):
        ...
        self.single_btn = self.action_button(actions, f"Create Single Mod ({self.single_ext})", self.create_single_mod, BUTTON_GOLD)
        self.single_btn.pack(side="left", padx=6)
        self.package_btn = self.action_button(actions, f"Create Package Mod ({self.package_ext})", self.create_package_mod, BUTTON_GREEN)
    ...
    def set_creator_busy(self, busy: bool):
        self.busy = busy
        state = "disabled" if busy else "normal"
        for attr in ("single_btn", "package_btn"):
    ...
    def create_mod(self, single_mode: bool):
        if self.busy:
            self.set_status("A package is already being written.", BUTTON_RED)
            return
        try:
        ...
        self.set_creator_busy(True)
        self.set_status(f"Writing {os.path.basename(save_path)}…", "#1F6B32")
        ...
        def done(_result):
            self.set_creator_busy(False)
            self.set_status(
                f"Created {os.path.basename(save_path)} with {len(payload_entries)} payload file(s), "
                f"{len(self.images_to_pack)} preview image(s), and "
                f"{'a WAV' if self.audio_to_pack else 'no WAV'}.",
                "#1F6B32",
            )
