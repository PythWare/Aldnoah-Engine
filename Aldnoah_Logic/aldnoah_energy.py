...
class NpcEditorSchema:
    ...
    name_field: str = "Name"
    name_list_path: str = ""
    names_use_slot_index: bool = False
    voice_map_path: str = ""
    model_map_path: str = ""
...
@dataclass(frozen=True)
class StageEditorSchema:
    game_id: str
    display_name: str
    source_root_rel: str
    export_dir: str
    stage_file_rels: Tuple[str, ...]
    fields: Tuple[str, ...]
    stage_names: Tuple[str, ...] = ()
    placeholder_prefix: str = "Stage"
    leader_field: str = "Squad Leader"
    offset: int = 0x7C
    slot_count: int = 120
    slot_size: int = 104
    hero_star_seed: int = 863
    hero_star_count: int = 46
    constellation_star_seed: int = 509
    constellation_star_count: int = 72
    constellation_arms: int = 10
...
GAME_SCHEMAS: Dict[str, GameSchema] = {
    ...
    "WO4": GameSchema(
        game_id="WO4",
        display_name="Warriors Orochi 4",
        containers=("LINKDATA.BIN",),
        idx_files=("LINKDATA.IDX",),
        unpack_folder="WO4_Unpacked",
    ),
    "BN": GameSchema(
        game_id="BN",
...
BN_NPC_FIELDS: Tuple[Tuple[str, int], ...] = (
    ("Squad Leader", 2),
    ("Squad ID", 2),
    ("Param 1", 2),
) + tuple((f"Unknown {index}", 1) for index in range(1, 27))
...
BN_NPC = NpcEditorSchema(
    game_id="BN",
    display_name="Bladestorm Nightmare",
    npc_count=1078,
    section=BinaryRecordSectionSchema(
        file_label="entry_06320.bin",
        file_path=os.path.join(PROJECT_ROOT, "BN_Unpacked", "Pack_00", "entry_06320.bin"),
        export_dir=os.path.join(PROJECT_ROOT, "BN_Unit_Edits", "Pack_00"),
        file_name="entry_06320.bin",
        offset=0x10,
        record_size=32,
        fields=BN_NPC_FIELDS,
        section_title="entry_06320.bin Core Data",
        section_subtitle="Mapped BN unit fields from the unit data block.",
        columns=2,
        hex_field_prefixes=("Unknown", "Param"),
    ),
    placeholder_prefix="Unit",
    name_field="Squad Leader",
    name_list_path=os.path.join(PROJECT_ROOT, "Aldnoah_Logic", "bn_doc", "bn_names.txt"),
    names_use_slot_index=True,
    hero_star_seed=911,
    hero_star_count=48,
    constellation_star_seed=967,
    constellation_star_count=90,
...
DW8XL_STAGE_FILE_RELS = tuple(
    os.path.join("DW8XL_Unpacked", "Pack_01", f"entry_{entry_id:05d}.bin")
    for entry_id in range(3063, 3407)
)

DW8XL_STAGE_NAMES = tuple(
    stage_names_dict.get(ordinal, "")
    for ordinal in range(1, len(DW8XL_STAGE_FILE_RELS) + 1)
)

DW7XL_STAGE_NAMES = load_text_name_list(
    os.path.join(PROJECT_ROOT, "Aldnoah_Logic", "dw7xl_doc", "stage_names.txt")
)

STAGE_EDITOR_SCHEMAS: Dict[str, StageEditorSchema] = {
    "DW7XL": StageEditorSchema(
        game_id="DW7XL",
        display_name="DW7XL",
        source_root_rel=os.path.join("DW7XL_Unpacked", "Pack_01"),
        export_dir=os.path.join(PROJECT_ROOT, "DW7XL_Stage_Edits", "Pack_01"),
        stage_file_rels=DW7XL_STAGE_FILE_RELS,
        fields=DW7XL_STAGE_FIELDS,
        stage_names=DW7XL_STAGE_NAMES,
        offset=0,
        slot_size=128,
        hero_star_seed=853,
        hero_star_count=46,
        constellation_star_seed=503,
        constellation_star_count=72,
    ),
    "DW8XL": StageEditorSchema(
        game_id="DW8XL",
        display_name="DW8XL",
        source_root_rel=os.path.join("DW8XL_Unpacked", "Pack_01"),
        export_dir=os.path.join(PROJECT_ROOT, "DW8XL_Stage_Edits", "Pack_01"),
        stage_file_rels=DW8XL_STAGE_FILE_RELS,
        fields=DW8XL_STAGE_FIELDS,
        stage_names=DW8XL_STAGE_NAMES,
        hero_star_seed=863,
        hero_star_count=46,
        constellation_star_seed=509,
        constellation_star_count=72,
    ),
}
