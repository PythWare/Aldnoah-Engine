...
def build_relative_pair_block(layout: dict, chunks: list[bytes]) -> bytes:
    ...
    for (rel_off, old_sz, old_abs_off), chunk in zip(original_entries, chunks):
        if old_sz > len(chunk):
...
def build_relative_pairtable_block(layout: dict, chunks: list[bytes]) -> bytes:
    ...
    written_ranges: list[tuple[int, int]] = []
    for (rel_off, old_sz, old_abs_off), chunk in zip(layout["entries"], chunks):
        if old_sz > len(chunk):
...
def rebuild_kshl_blob_from_folder(folder_path: str, original_raw: bytes) -> bytes:
    ...
    for path in payload_files:
        ...
        if ext in (".vsh", ".psh") and not detect_dx9_shader_ext(chunk, 0):
            raise ValueError(f"{os.path.basename(path)} doesnt look like a DX9 shader bytecode blob.")
...
def rebuild_kshl_from_folder(
    ...
    if not os.path.isdir(folder_path):
        raise ValueError("Selected KSHL folder doesnt exist.")
    if not os.path.isfile(original_kshl_path):
        raise ValueError("Selected original KSHL file doesnt exist.")
...
def rebuild_kvs_blob_from_folder(folder_path: str) -> bytes:
    ...
    if not kvs_files:
        raise ValueError("Selected KVS folder doesnt contain any .kvs files to rebuild.")
...
def rebuild_mdlk_from_folder(
    ...
    if not os.path.isdir(folder_path):
        raise ValueError("Selected MDLK folder doesnt exist.")
    if not os.path.isfile(original_mdlk_path):
        raise ValueError("Selected original MDLK file doesnt exist.")
...
def rebuild_embedded_mdlk_blob_from_folder(folder_path: str, original_raw: bytes) -> bytes:
    ...
    if not layout:
        raise ValueError("Original file doesnt contain supported embedded MDLK resources.")
...
def rebuild_embedded_mdlk_from_folder(
    ...
    if not os.path.isdir(folder_path):
        raise ValueError("Selected embedded MDLK folder doesnt exist.")
    if not os.path.isfile(original_resource_path):
        raise ValueError("Selected original embedded MDLK wrapper doesnt exist.")
...
def rebuild_classic_split_zlib_raw_from_folder(folder_path: str, original_raw: bytes) -> bytes:
    ...
    if not layout:
        raise ValueError("Original file doesnt look like a classic split-zlib resource.")
...
def rebuild_universal_subcontainer_raw_from_folder(folder_path: str, original_raw: bytes) -> bytes:
    ...
    if not layout:
        raise ValueError("Original file doesnt look like a supported universal subcontainer.")

    folder_files = list_folder_payload_files(folder_path)
    if not folder_files:
        raise ValueError("Selected subcontainer folder doesnt contain any files to rebuild.")
...
def rebuild_classic_split_zlib_from_folder(folder_path: str, original_resource_path: str, output_path: str | None = None):
    if not os.path.isdir(folder_path):
        raise ValueError("Selected split-zlib folder doesnt exist.")
    if not os.path.isfile(original_resource_path):
        raise ValueError("Selected original split-zlib file doesnt exist.")
...
def rebuild_split_zlib_wrapper_from_folder(folder_path: str, original_resource_path: str, output_path: str | None = None):
    if not os.path.isdir(folder_path):
        raise ValueError("Selected split-zlib wrapper folder doesnt exist.")
    if not os.path.isfile(original_resource_path):
        raise ValueError("Selected original split-zlib wrapper file doesnt exist.")
...
def rebuild_subcontainer_from_folder(folder_path: str, original_subcontainer_path: str, output_path: str | None = None):
    if not os.path.isdir(folder_path):
        raise ValueError("Selected subcontainer folder doesnt exist.")
    if not os.path.isfile(original_subcontainer_path):
        raise ValueError("Selected original subcontainer file doesnt exist.")
...
def unpack_from_schema(
    ...
    write_files False produces only the taildata manifest: every entry is still
    read and decoded so the recorded names and sizes match a real unpack exactly,
    ...
    if write_files and not os.path.isdir(out_root):
    ...
    update_status(
        f"{'Unpacking' if write_files else 'Scanning'} {game_name} "
        f"(entry size: {entry_size} bytes, schema-driven)",
        "blue",
    )
    ...
    try:
        ...
        update_status(
            f"The taildata manifest could not be saved: {exc}",
            "red",
        )
        return None

    update_status(
        f"Finished {'unpacking' if write_files else 'scanning'} {game_name}.",
        "green",
    )
...
def unpack_pair(
    ...
    *,
    ...
    With write_files False nothing is written and no nested resources are walked;
    every entry is still read, descrambled and decompressed so the manifest comes
    out identical to a real unpack
    """
        ...
        try:
            ...
            for i in range(total_entries):
                ...
                if cipher is not None and cipher.applies_to(flag):
                    entry_index = cipher.entry_index_from_offset(
                        start_from_offset + start, entry_size
                    )
                    native_fn = getattr(cipher, "transform_fast_from_container", None)
                    if native_fn is not None:
                        raw = native_fn(bin_path, offset, size_to_read, entry_index)
                    if raw is None:
                        raw = cipher.transform(mm[offset:offset + size_to_read], entry_index)
                if raw is None:
    ...
    update_status(
        f"{'Unpacked' if write_files else 'Scanned'} {total_entries} IDX entries "
        f"from {os.path.basename(bin_path)}",
        "green",
    )
...
def unpack_multi_containers(
    ...
    *,
    ...
    try:
        for i in range(total_entries):
            ...
            while offset + size_to_read > current_size:
                if not advance_container():
                    update_status(
                        ...
                        f"doesnt fit in remaining containers; skipping.",
                        "red",
                    )
                    size_to_read = 0
                    break
            ...
            if cipher is not None and cipher.applies_to(flag):
                ...
                native_fn = getattr(cipher, "transform_fast_from_container", None)
                if native_fn is not None:
                    raw = native_fn(bin_paths[current_idx], offset, size_to_read, entry_index)
                if raw is None:
                    raw = cipher.transform(current_map[offset:offset + size_to_read], entry_index)
            if raw is None:
            ...
            if write_files and not os.path.isdir(container_out_dir):
    ...
    update_status(
        f"{'Unpacked' if write_files else 'Scanned'} {total_entries} IDX entries "
        f"across {len(bin_files)} containers.",
        "green",
    )
...
def generate_taildata_manifest(
    schema: GameSchema,
    base_dir: str,
    status_callback=None,
    progress_callback=None,
    ...
    """
    Build the taildata manifest for a game without unpacking it
    """
    return unpack_from_schema(
        schema,
        base_dir,
