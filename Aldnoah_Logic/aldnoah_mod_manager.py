...
import ctypes, math, os, json, shutil, hashlib, random, threading
...
from .aldnoah_tools import on_main_thread, run_in_background
...
ALDNOAH_FORMAT_VERSION = 4
ALDNOAH_COMPATIBLE_FORMAT_VERSIONS = {2, 3, 4}
...
def installer_payload_to_entry(payload) -> ModFileEntry:
    """
    Turn one installer payload into an entry
    """
    entry_off = int(getattr(payload, "entry_off", 0) or 0)
    if entry_off > 0:
        tail = TailData(int(getattr(payload, "idx_marker", 0) or 0),
                        entry_off,
                        int(getattr(payload, "comp_marker", 0) or 0))
        return ModFileEntry(stored_name=payload.stored_name, payload=payload.data, tail=tail)
    ...
    def read(self, *, include_payloads: bool = True, include_media: bool = True) -> ParsedModPackage:
        with open(self.path, "rb") as f:
            ...
            for idx in range(file_count):
                ...
                if format_version >= 4:
                    idx_marker = read_u8(f, f"entry {idx + 1} container")
                    entry_off = read_u32(f, f"entry {idx + 1} slot offset")
                    comp_marker = read_u8(f, f"entry {idx + 1} compression marker")
                    sz = read_u32(f, f"entry {idx + 1} payload size")
                    if not include_payloads:
                        f.seek(sz, os.SEEK_CUR)
                        continue
                    payload = read_exact(f, sz, f"entry {idx + 1} payload")
                    tail = TailData(idx_marker, entry_off, comp_marker)
                else:
                    sz = read_u32(f, f"entry {idx + 1} payload size")
                    if not include_payloads:
                        f.seek(sz, os.SEEK_CUR)
                    ...
                    tail = TailData.parse(blob[-TAILDATA_LEN:], endian="little")
...
class ModManagerWindowV2(tk.Toplevel):
    ...
    def installer_targets_from_package(self, path: str) -> Set[Tuple[int, int]]:
        ...
        try:
            ...
            for payload in package.payloads:
                try:
                    entry = installer_payload_to_entry(payload)
                except ValueError:
                    continue
                targets.add((entry.tail.idx_marker, entry.tail.entry_off))
        except Exception:
            targets = set()
        ...
        try:
            ...
            for payload in package.payloads:
                try:
                    ...
                    continue
                key = (entry.tail.idx_marker, entry.tail.entry_off)
                details.setdefault(key, set()).add(
                    payload.stored_name or payload.source_name or "installer payload")
        except Exception:
            details = {}
    ...
    def build_gui(self):
        ...
        self.disable_all_btn = tk.Button(top, text="Disable All", command=self.disable_all_async, width=12)
        self.disable_all_btn.pack(side=tk.LEFT, padx=6, pady=10)
    ...
    def set_status(self, text: str, color: str = "green"):
        if threading.current_thread() is not threading.main_thread():
            on_main_thread(self, self.set_status, text, color)
            return
    ...
    def show_error_safe(self, title: str, message: str):
        """messagebox.showerror, safe to call from the background write loop"""
        on_main_thread(self, messagebox.showerror, title, message)

    def set_ui_busy(self, busy: bool):
        self.busy = busy
        state = "disabled" if busy else "normal"
        for attr in ("enable_btn", "disable_btn", "disable_all_btn"):
    ...
    def rescan_and_render(self, status: Optional[str] = None):
        if threading.current_thread() is not threading.main_thread():
            on_main_thread(self, self.rescan_and_render, status)
            return
    ...
    def apply_mod_file(self, path: str):
        ...
        if not self.require_ready():
            return
        if self.busy:
            self.set_status("Another apply/disable is still running.", "red")
            return
        if self.ledger.is_enabled(filename):
            self.set_status(f"'{filename}' is already enabled. Disable it first to reapply.", "blue")
            return
        self.set_ui_busy(True)
        self.set_status(f"Reading {filename}", "blue")

        def parse():
            return ModParser(path).read(include_media=False).entries
        ...
        def parsed(entries):
            ...
            if not self.confirm_apply_collisions(filename, targets):
                ...
                return
            self.apply_entries_async(
                filename, entries,
                on_complete=lambda ok: self.rescan_and_render(
                    status=f"Applied {filename}." if ok else None
                ),
                continuing=True,
            )
        ...
        def failed(exc):
            self.set_ui_busy(False)
            self.set_status(f"Failed to read {filename}.", "red")
            messagebox.showerror("Mod Parse Error", str(exc))

        run_in_background(self, parse, on_done=parsed, on_error=failed)

    def apply_entries_async(self, filename: str, entries: List[ModFileEntry], on_complete=None,
                            continuing: bool = False):
        """
        Apply entries without blocking the window
        """
        if self.busy and not continuing:
            self.set_status("Another apply/disable is still running.", "red")
            if on_complete is not None:
                on_complete(False)
            return

        idx_markers = sorted({ent.tail.idx_marker for ent in entries})
        for idx_marker in idx_markers:
            if not self.prompt_for_container(idx_marker):
                self.set_ui_busy(False)
                self.set_status("Apply cancelled.", "red")
                if on_complete is not None:
                    on_complete(False)
                return
            if not self.resolve_idx_path(idx_marker):
                self.set_ui_busy(False)
        ...
        self.set_ui_busy(True)
        self.set_status(f"Applying {filename} (0/{len(entries)})", "blue")

        def work():
            return self.apply_entries(filename, entries)
        ...
        def failed(exc):
            ...
            self.show_error_safe("Apply Error", f"Unexpected error applying '{filename}':\n{exc}")
            if on_complete is not None:
                on_complete(False)
            ...
            for ent in grouped_entries:
                try:
                    ...
                    if cipher is not None:
                        ...
                        transform_fn = getattr(cipher, "transform_fast", cipher.transform)
                        payload = transform_fn(payload, slot)
                    ...
                    return False
                total_done += 1
                if total_done == total or (total_done & 15) == 0:
                    self.set_status(f"Applying {filename} ({total_done}/{total})", "blue")
    ...
    def disable_selected(self):
        ...
        if not self.require_ready():
            return
        self.disable_mod_async(self.selected.filename)

    def disable_mod_async(self, mod_name: str, on_complete=None):
        """
        disable_mod, without blocking the window while the IDX entries restore
        """
        if self.busy:
            self.set_status("Another apply/disable is still running.", "red")
            if on_complete is not None:
        ...
        markers = sorted({
            idx_marker for name, idx_marker, *_rest in self.ledger.iter_records()
            if name and name.strip().lower() == target
        })
        ...
        self.set_ui_busy(True)
        self.set_status(f"Disabling {mod_name}", "blue")

        def work():
            return self.disable_mod(mod_name)
        ...
        def failed(exc):
            ...
            self.show_error_safe("Disable Error", f"Unexpected error disabling '{mod_name}':\n{exc}")
            if on_complete is not None:
                on_complete(False)
    ...
    def disable_mod(self, mod_name: str):
        ...
        for name, idx_marker, entry_off, entry_size, entry_bytes in self.ledger.iter_records():
            ...
            try:
                ...
                self.show_error_safe("Disable Error", f"Failed restoring an IDX entry:\n{e}")
                self.set_status("Disable failed (partial changes may remain).", "red")
                return False
        ...
        try:
            ...
            self.show_error_safe("Ledger Error", f"Failed rewriting ledger:\n{e}")
            return False
    ...
    def disable_all(self, confirm: bool = True):
        if not self.require_ready():
            return
        ...
        for name, idx_marker, entry_off, entry_size, entry_bytes in self.ledger.iter_records():
            ...
            try:
                ...
                self.show_error_safe("Disable All Error", f"Failed restoring an IDX entry:\n{e}")
    ...
    def disable_all_async(self):
        """
        disable_all, without blocking the window while every entry restores
        """
        if self.busy:
            self.set_status("Another apply/disable is still running.", "red")
            return
        ...
        markers = sorted({idx_marker for name, idx_marker, *_rest in self.ledger.iter_records() if name})
        for idx_marker in markers:
            if not self.resolve_idx_path(idx_marker):
                self.set_status(f"Missing IDX for BIN {idx_marker}; cannot restore all.", "red")
                return
        ...
        self.set_ui_busy(True)
        self.set_status("Disabling all mods", "blue")

        def work():
            self.disable_all(confirm=False)
        ...
        def failed(exc):
            self.set_ui_busy(False)
            self.set_status("Disable All failed (partial changes may remain).", "red")
            self.show_error_safe("Disable All Error", f"Unexpected error: {exc}")
...
class StandardInstallerWindow(tk.Toplevel):
    def __init__(self, manager: ModManagerWindowV2, path: str):
        ...
        self.title(f"Loading {self.filename}")
        ...
        self.loading_label = tk.Label(
            self, text=f"Reading {self.filename}", bg=ORRERY_BG, fg=TEXT,
            font=("Segoe UI", 12, "bold"),
        )
        self.loading_label.pack(expand=True)

        def load():
            return AldnoahInstallerReader().read(
                path,
                include_blobs=False,
                include_asset_blobs=True,
                include_payload_blobs=True,
        ...
        run_in_background(self, load, on_done=self.on_package_loaded, on_error=self._on_load_failed)

    def _on_load_failed(self, exc):
        messagebox.showerror("Installer Error", f"Could not read {self.filename}:\n{exc}", parent=self.manager)
        self.destroy()

    def on_package_loaded(self, package):
    ...
    def build_ui(self):
        ...
        self.install_btn = tk.Button(footer, text="Install Package", command=self.run_install, width=20)
        self.install_btn.pack(side=tk.RIGHT, padx=(8, 14), pady=10)
    ...
    def set_install_busy(self, busy: bool):
        self.busy = busy
        try:
            self.install_btn.config(state="disabled" if busy else "normal")
        except Exception:
            pass
    ...
    def run_install(self):
        ...
        if self.manager.ledger.is_enabled(self.filename):
            ...
            if not ok:
                return
            self.set_install_busy(True)
            self.manager.disable_mod_async(self.filename, on_complete=self.after_disable_for_install)
            return

        self.set_install_busy(True)
        self._install_entries()

    def after_disable_for_install(self, ok: bool):
        if ok:
            self._install_entries()
        else:
            self.set_install_busy(False)

    def _install_entries(self):
        self.manager.apply_entries_async(self.filename, self.entries, on_complete=self.on_install_applied)

    def on_install_applied(self, ok: bool):
        self.set_install_busy(False)
        if not ok:
            return
...
class InstallerWizardWindow(tk.Toplevel):
    ...
    def on_package_loaded(self, package):
        ...
        self.icon_blob = self.first_asset_blob("icon")
        self.banner_blob = self.first_asset_blob("banner")
        saved = self.manager.load_installer_state().get(self.filename, {})
    ...
    def build_ui(self):
        ...
        footer.pack(side=tk.BOTTOM, fill=tk.X)
        footer.pack_propagate(False)
        self.install_btn = tk.Button(footer, text="Install Selected Payloads", command=self.run_install, width=24)
        self.install_btn.pack(side=tk.RIGHT, padx=(8, 14), pady=10)
        tk.Button(footer, text="Cancel", command=self.destroy, width=12).pack(side=tk.RIGHT, padx=8, pady=10)
    ...
    def run_install(self):
        ...
        def install_entries():
            self.manager.apply_entries_async(self.filename, entries, on_complete=on_applied)

        def on_applied(ok: bool):
            self.set_install_busy(False)
            if not ok:
                return
        ...
        def after_disable(disabled: bool):
            if disabled:
                install_entries()
            else:
                self.set_install_busy(False)
        ...
        if self.manager.ledger.is_enabled(self.filename):
            ...
            if not ok:
                return
            self.set_install_busy(True)
            self.manager.disable_mod_async(self.filename, on_complete=after_disable)
            return
