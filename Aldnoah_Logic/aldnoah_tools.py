...
def on_main_thread(widget, func, *args) -> None:
    """
    ...
    if threading.current_thread() is threading.main_thread():
        ...
        return
    try:
        widget.after(0, func, *args)
    except Exception:
        pass


def run_in_background(widget, work, on_done=None, on_error=None) -> None:
    """
    Run work on a daemon thread, delivering the result back on the Tk thread
    """
    ...
    def runner():
        try:
            ...
            if on_error is not None:
                on_main_thread(widget, on_error, exc)
            return
        if on_done is not None:
            on_main_thread(widget, on_done, result)
...
def diagnose_aldnoah_directory(
    ...
    *,
    ...
    try:
        ...
        issues.append(diagnostic_issue("error", "disk_usage_failed", f"Couldnt read drive storage: {exc}"))
    ...
    if not exists:
        ...
        if not can_read:
            issues.append(diagnostic_issue("error", "read_denied", f"Couldnt read the directory: {read_error}"))
        if not can_write_file:
            issues.append(diagnostic_issue("error", "write_denied", f"Couldnt create/write a test file: {file_error}"))
        elif not file_delete_ok:
            issues.append(diagnostic_issue("error", "delete_file_denied", f"Couldnt delete a test file: {file_error}"))
        if not can_create_directory:
            issues.append(diagnostic_issue("error", "mkdir_denied", f"Couldnt create a test folder: {dir_error}"))
        elif not dir_delete_ok:
            issues.append(diagnostic_issue("error", "delete_dir_denied", f"Couldnt delete a test folder: {dir_error}"))
...
def make_probe_directory(path: str) -> str:
    for attempt in range(32):
        ...
        try:
            ...
            continue
    raise FileExistsError("Couldn't reserve a unique diagnostic test folder name.")
...
def protected_location_name(path: str) -> str:
    ...
    driv, tail = os.path.splitdrive(os.path.abspath(path))
