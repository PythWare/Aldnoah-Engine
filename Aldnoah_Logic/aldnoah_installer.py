...
INSTALLER_FORMAT_VERSION = 2
INSTALLER_COMPATIBLE_VERSIONS = {1, 2}
...
class InstallerPayload:
    ...
    def record(self) -> dict:
        return {
            "idx_marker": self.idx_marker,
            "entry_off": self.entry_off,
            "comp_marker": self.comp_marker,
        }
...
class AldnoahInstallerWriter:
    ...
    def build_payload_section(payloads: Iterable[InstallerPayload]) -> bytes:
        ...
        for payload in payload_list:
            ...
            buf.write(digest)
            write_u8(buf, int(payload.idx_marker) & 0xFF)
            write_u32(buf, int(payload.entry_off) & 0xFFFFFFFF)
            write_u8(buf, 1 if payload.comp_marker else 0)
...
class AldnoahInstallerReader:
    def read(
        ...
        *,
        ...
        with open(path, "rb") as handle:
            ...
            if version not in INSTALLER_COMPATIBLE_VERSIONS:
            ...
            for _ in range(section_count):
                ...
                if tag == "META":
                    ...
                    payloads = self.read_payload_section(section, include_blobs=bool(include_payload_blobs), version=version)
                else:
    ...
    @staticmethod
    def read_payload_section(data, *, include_blobs: bool = True,
                             version: int = INSTALLER_FORMAT_VERSION) -> List[InstallerPayload]:
        handle = BytesIO(data) if isinstance(data, (bytes, bytearray)) else data
        ...
        for idx in range(count):
            ...
            marker = off = comp = 0
            if version >= 2:
                marker = read_u8(handle, f"payload {idx + 1} container")
                off = read_u32(handle, f"payload {idx + 1} slot offset")
                comp = read_u8(handle, f"payload {idx + 1} compression marker")
...
class InstallerCreatorWindow(tk.Toplevel):
    ...
    def payload_with_record(self, path: str) -> Tuple[bytes, dict]:
        """
        Read a staged file and look up the slot it targets

        Nothing is appended: the installer stores the slot as its own fields
        """
    ...
    def collect_payloads(self) -> Tuple[List[InstallerPayload], Dict[str, str]]:
        ...
        for data in self.arch_data.values():
            ...
            for path in data.get("payload_paths", []):
                ...
                raw, record = self.payload_with_record(path)
                ...
                payloads.append(InstallerPayload(
                    payload_id, stored_name, base, hashlib.sha256(raw).hexdigest(), raw, path,
                    int(record["idx_marker"]), int(record["entry_off"]),
